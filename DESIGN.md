...
**Hamburger toggle.** A 2026-07-31 addition, not present in the 2020 source. A 44x44 button (the WCAG 2.5.5 target-size floor, matching the section links' own `min-height`) with square corners like every other rectangular control in the system, a transparent fill, a `{colors.controlBorder}` boundary and a three-line icon in `{colors.text}`; on hover the border shifts to `{colors.text}`. It appears at the exact inverse of the section links' breakpoint, so the header is never without navigation. Opening it drops a full-width panel beneath the bar in `{colors.ground}` with a `{colors.overlayMedium}` bottom border. The panel closes on click-again, Escape, a click outside, and on activating a link; the first three return focus to the toggle, the fourth sends focus to the target section (which carries `tabindex="-1"` for that purpose). State is carried by `aria-expanded`, never by color alone.
